...
let pause = false;
let GameOver = false;
let fagyujjatek=false;//h jol mukodjon az anim ez ahhoz segit
let lehetosegek = 3;
let pontszam = 0;
...
function ElsoSzint() {
    ...
    level_timer = setInterval(() => {
        ...
        console.log("kód eltűnése: " + kodeltunes);
    }
    }, rndtime);
    }
...
function IdoFagyasztas() {
    if (fagyasztas == false && lassitvolte == false && hatralevo_ido < 120) {
        fagyasztas = true;
        fagyasztvolte = true;
        fagyashoz.classList.remove("fagyasel")
        fagyashoz.classList.remove("fagyasmeg");
        ...
        pause = true;
        setTimeout(() => {
            //if-es resz az uj es ami volt a regi if-ben az most else if
